...
from marineanomalydetection.dataset.aggregator import (
    aggregate_to_multi,
    aggregate_to_binary,
    aggregate_to_11_classes,
)
from marineanomalydetection.dataset.get_rois import get_rois
from marineanomalydetection.dataset.update_count_labeled_pixels import (
    update_count_labeled_pixels,
)
...
class MarineAnomalyDataset(Dataset, ABC):
    def __init__(
        ...
        self._check_weak_transform_unlabeled_version_one_train_set(
            weak_transform_unlabeled_version_one_train_set, mode
        )

        if (
            mode == DataLoaderType.TRAIN_SET_SUP
            or mode == DataLoaderType.TRAIN_SET_SUP_AND_UNSUP
        ):
        ...
        self.impute_nan = np.tile(BANDS_MEAN, (MARIDA_SIZE_X, MARIDA_SIZE_Y, 1))
    ...
    def _check_weak_transform_unlabeled_version_one_train_set(
        ...
        if (
            ...
            raise Exception(
                "The weak_transform_unlabeled_version_one_train_set has to be used only when training with SSL with only 1 training set."
            )
        if (
            weak_transform_unlabeled_version_one_train_set is None
            and mode is DataLoaderType.TRAIN_SET_SUP_AND_UNSUP
        ):
            raise Exception(
                "The weak_transform_unlabeled_version_one_train_set should not be set to None with the chosen mode."
            )

    @staticmethod
    def _load_and_process_and_add_patch_to_dataset(
        patch_path: str, list_patches: list[np.ndarray]
    ) -> None:
    ...
    def _load_and_process_and_add_seg_map_to_dataset(
        ...
        if perc_labeled is not None:
            num_pixels_dict = update_count_labeled_pixels(
                seg_map, aggregate_classes, self.categories_counter_dict
            )
    ...
    def _apply_transform_to_patch_and_seg_map(
        self, patch: np.ndarray, seg_map: np.ndarray
