...
from marineanomalydetection.dataset.assert_percentage_categories import (
    ...
    def load_data(self):
        ...
        for roi in tqdm(
            ...
            num_pixels_dict = self._load_and_process_and_add_seg_map_to_dataset(
                seg_map_path, self.y, self.aggregate_classes, self.perc_labeled
            )
            # Loads patch
            self._load_and_process_and_add_patch_to_dataset(patch_path, self.X)
        ...
        if (
            self.mode == DataLoaderType.TRAIN_SET_SUP
            or self.mode == DataLoaderType.TRAIN_SET_SUP_AND_UNSUP
        ) and self.perc_labeled is not None:
            assert_percentage_categories(
                self.categories_counter_dict, self.perc_labeled, num_pixels_dict
            )
